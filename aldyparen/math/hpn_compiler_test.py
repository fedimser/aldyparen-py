...
def check_bivariate(expr: str, golden: Callable[[complex, complex], complex]):
    ...
    for _ in range(10):
        ...
        ans = ComplexHpn.from_raw(
            evaluator(ComplexHpn.from_number(a).to_raw(), ComplexHpn.from_number(b).to_raw())
        ).to_complex()
...
def test_bivariate():
    ...
    check_bivariate("(a**2*b)+(2.0*a*3.0*b**2)+a", lambda a, b: (a**2 * b) + (2.0 * a * 3.0 * b**2) + a)
    # Burning ship.
